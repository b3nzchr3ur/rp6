...
void test_led_value_is_updated_when_initially_off(void)
{
	...
	uint8_t new_led_value = 0b000;

	led_values_update(led_value, &new_led_value);

	TEST_ASSERT_EQUAL(0b0001, new_led_value);
}

void test_led_value_is_updated_when_already_on(void)
{
	uint8_t led_value = 0b0100;
	uint8_t new_led_value = 0;

	led_values_update(led_value, &new_led_value);

	TEST_ASSERT_EQUAL(0b1000, new_led_value);
}
