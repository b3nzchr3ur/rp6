...
// calibratiewaarden ten behoeve van drift correctie.
// Als robot horizontaal staat, moet X = Y = 0 zijn. En Z = 63 (+1g).

const int16_t      Xcal = 52;
const int16_t      Ycal = 62;
const int16_t      Zcal = 6;


//static uint16_t   XAccel;   // Variables to store incoming RAW data from the accelerometer
//static uint16_t   YAccel;
//static uint16_t   ZAccel;

#define deviceAddress 0x1D
#define writeAddress   (deviceAddress << 1)
#define readAddress   ((deviceAddress << 1) | 0x01)
...
void MMA7455_set_to_standby_mode() {
	// Vul hier zelf de juiste code in (hint: denk aan de datasheet)
}

void MMA7455_set_to_measurement_mode() {
	// Vul hier zelf de juiste code in (hint: denk aan de datasheet)
}
